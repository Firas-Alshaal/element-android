...
data class GroupMapViewState(
        ...
        val currentMapStyle: String = "hybrid",
        val availableMapStyles: List<String> = emptyList(),
...
sealed class GroupMapAction : VectorViewModelAction {
        ...
        data class SwitchMapStyle(val style: String) : GroupMapAction()
        data class MapStyleChanged(val style: String) : GroupMapAction()
}
...
sealed class GroupMapViewEvents : VectorViewEvents {
        ...
        data class MapStyleSwitched(val style: String) : GroupMapViewEvents()
...
class GroupMapViewModel @AssistedInject constructor(
        ...
        init {
                // Initialize available map styles
                val availableStyles = vtpkMapProvider.getAvailableStyles()
                setState { copy(availableMapStyles = availableStyles) }
        }
        ...
        override fun handle(action: GroupMapAction) {
                when (action) {
                        ...
                        is GroupMapAction.SwitchMapStyle -> handleSwitchMapStyle(action.style)
                        is GroupMapAction.MapStyleChanged -> handleMapStyleChanged(action.style)
                }
        }
        ...
        fun getMapPathForStyle(style: String): String? {
                return vtpkMapProvider.getMapPathForStyle(style)
        }
                        ...
                        try {
                                ...
                                setState { 
                                        copy(
                                                isLoading = false, 
                                                isMapLoaded = true,
                                                currentMapStyle = vtpkMapProvider.getCurrentStyle(),
                                                error = null
                                        ) 
                                }
        ...
        private fun handleSwitchMapStyle(style: String) {
                try {
                        if (vtpkMapProvider.setCurrentStyle(style)) {
                                setState { copy(currentMapStyle = style) }
                                _viewEvents.post(GroupMapViewEvents.MapStyleSwitched(style))
                                Timber.d("Map style switched to: $style")
                        } else {
                                Timber.e("Invalid map style: $style")
                        }
                } catch (e: Exception) {
        ...
        private fun handleMapStyleChanged(style: String) {
                setState { copy(currentMapStyle = style) }
        }
